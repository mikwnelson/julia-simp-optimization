...
\begin{equation}
	...
	\end{cases}
\end{equation}
Though complex, it is possible to compute $\displaystyle\frac{\partial f_{av}}{\partial\eta_{\ell}}$ using the above. Now we need to show how to find $\displaystyle\frac{\partial\overline{\phi}}{\eta_{\ell}}$.

The porosity function is
\begin{equation}
	\phi(\boldsymbol{\eta})=\frac{1}{N_\eta}\mathbf{1}^T\boldsymbol{\eta}\label{eqn:porosity}
\end{equation}
where $N_\eta$ indicates the number of design volumes. It is easy to compute the gradient of \eqref{eqn:porosity}:
\begin{equation}
	\frac{\partial\phi}{\partial\eta_{\ell}}=\frac{1}{N_\eta}.\label{eqn:porosity-deriv}
\end{equation}